...
# Velocity grid file 
vGridFileName = '/Data/gfi/scratch/johor1356/IBI_MULTIYEAR_PHY_005_002/IBI_MULTIYEAR_PHY_005_002-TDS_201512.nc'
...
velocityFiles=sorted(glob.glob('/Data/gfi/scratch/johor1356/IBI_MULTIYEAR_PHY_005_002/IBI_MULTIYEAR_PHY_005_002-TDS_*.nc'))# + 
...
#fData = xr.open_dataset(particlesFile)
    ...
    for m in np.arange(1,fData.dims['obs']):#fData.dims['obs']): # Loop over times
      ...
      fTraj = np.reshape(fData.trajectory[:].values,(nLat,nLon))
...
fsleData = xr.Dataset(
                ...
                coords={
                    "latitude":("latitude",fLat.data),
                    "longitude":("longitude",fLon.data),
                    "time":("time",fDates)
                }
)
